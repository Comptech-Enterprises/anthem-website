...
- **`src/app/page.tsx`** is the entire page — a flat composition of section components in `src/components/`. Section order and which sections exist are controlled here. Adding/removing a section means editing this file.
...
- **Navigation is anchor-based.** Nav links and CTAs point to section `id`s (`#about`, `#work`, `#enquiry`, …). `SmoothScroll.tsx` intercepts anchor clicks for controlled-duration scrolling with a header offset; `globals.css` keeps `scroll-padding-top` only as a no-JS fallback. Keep section `id`s stable when refactoring.
- **`RibbonFlight.tsx`** wraps the About→Services→Work group and draws a scroll-linked paper-plane trail over them via an SVG path (`preserveAspectRatio="none"`, so tangent angles are computed in screen space). It's purely decorative but tightly coupled to those three sections being grouped together in `page.tsx`.
